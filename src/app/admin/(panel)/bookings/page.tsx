      ...
      render: (b: Booking) => (
        b.scheduledDate ? (
          <div className="text-white/60 text-sm">
            <p className="flex items-center gap-1"><Calendar size={11} className="text-[#d4af37]" /> {b.scheduledDate}</p>
            {b.scheduledTime && <p className="flex items-center gap-1 mt-0.5"><Clock size={11} className="text-[#d4af37]" /> {b.scheduledTime}</p>}
          </div>
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-3 sm:grid-cols-6 gap-3">
        ...
        <Link href="/admin/bookings/calendar"
          ...
          <p className="text-[9px] uppercase tracking-[0.25em] text-[#d4af37]">Calendar</p>
        </Link>
      </div>
        ...
        {selectedBooking && (
          <div className="space-y-5">
            ...
            <div className="grid grid-cols-2 gap-4">
              <div className="rounded-xl p-4" style={{ background: 'rgba(212,175,55,0.08)', border: '1px solid rgba(212,175,55,0.15)' }}>
                <p className="text-[9px] uppercase tracking-[0.35em] font-bold text-[#d4af37] mb-2">Customer</p>
              ...
              <div className="rounded-xl p-4" style={{ background: 'rgba(212,175,55,0.08)', border: '1px solid rgba(212,175,55,0.15)' }}>
                <p className="text-[9px] uppercase tracking-[0.35em] font-bold text-[#d4af37] mb-2">Current Status</p>
            ...
            {/* Services */}
            <div>
              <p className="text-[9px] uppercase tracking-[0.35em] font-bold text-[#d4af37] mb-3">Services Requested</p>
              <div className="space-y-2 max-h-40 overflow-y-auto">
                {selectedBooking.services?.map((svc, i) => (
                  <div key={i} className="flex items-center justify-between px-4 py-2.5 rounded-xl"
                    ...
                    <span className="text-[#d4af37] text-sm">{svc.member}</span>
                  </div>
                ))}
              </div>
            </div>
            ...
            {/* Schedule */}
            <div>
              <p className="text-[9px] uppercase tracking-[0.35em] font-bold text-[#d4af37] mb-3">Schedule Appointment</p>
              <div className="grid grid-cols-2 gap-4">
            ...
            {/* Status actions */}
            <div>
              <p className="text-[9px] uppercase tracking-[0.35em] font-bold text-[#d4af37] mb-3">Update Status</p>
              <div className="flex flex-wrap gap-2">
                {(['confirmed', 'completed', 'cancelled', 'rescheduled'] as const).map(status => (
                  <button key={status} onClick={() => updateBooking(selectedBooking.id, {
                    ...
                    style={{
                      background: selectedBooking.status === status ? 'rgba(212,175,55,0.2)' : 'rgba(255,255,255,0.06)',
                      border: `1px solid ${selectedBooking.status === status ? 'rgba(212,175,55,0.4)' : 'rgba(255,255,255,0.08)'}`,
                      color: selectedBooking.status === status ? '#d4af37' : 'rgba(255,255,255,0.5)',
                    }}
                  >{status}</button>
                ))}
              </div>
