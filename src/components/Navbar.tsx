    ...
    <>
      ...
      <nav
        className={`fixed top-0 left-0 right-0 z-50 transition-all duration-700 ease-in-out ${
          scrolled
            ? 'pt-3 pb-3 bg-[rgba(255,255,255,0.97)] backdrop-blur-[32px] border-b border-[rgba(212,175,55,0.12)] shadow-[0_4px_30px_rgba(31,53,94,0.08)]'
        ...
        <div className="container mx-auto px-4 md:px-6 flex items-center justify-between">
            ...
            {navLinks.map((link) => (
              <Link
                ...
                className={`nav-link transition-colors duration-500 hover:text-[#d4af37] ${
                  scrolled ? 'text-[#1f355e]/80' : 'text-white/90'
                }`}
              >
                {link.name}
              </Link>
          ...
          <button
            className={`md:hidden p-2 hover:text-[#d4af37] transition-colors duration-300 ${
              scrolled ? 'text-[#1f355e]' : 'text-white'
            }`}
              ...
              <div className="flex flex-col items-center justify-center h-full space-y-8 relative z-10">
                {navLinks.map((link, i) => (
                  ...
                  >
                    <Link
                      href={link.href}
                      className="font-headline text-2xl font-light text-[#1f355e]/80 hover:text-[#d4af37] transition-colors duration-300 tracking-[0.08em]"
